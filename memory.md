...
| 2026-07-08 | [2026-07-08-monty-hall](./ledger/2026-07-08-monty-hall.md) | #021 · game | IG carousel (5) · Threads card+thread (4 imgs) | publish-pending (manual), Buffer MCP connector absent + raw API egress-blocked (403 at proxy, verified); images hosted + verified, payloads in ledger |
...
| 2026-07-16 | [2026-07-16-mandelbrot](./ledger/2026-07-16-mandelbrot.md) | #029 · research | IG carousel (5) · Threads card+thread (4 imgs) | publish-pending (manual), Buffer MCP connector absent + raw API egress-blocked (curl 56 / http 000 at proxy, verified); images hosted + verified, payloads in ledger. (#025 grundy-values still skipped, no row; forge separately if wanted) |
...
| 2026-08-11 | [2026-08-11-gauss-circle](./ledger/2026-08-11-gauss-circle.md) | #055 · research | Threads thread (4 imgs, scheduled) · IG ig-pending | **posted** — Threads scheduled via Postiz MCP, postId `cmsoiibg00000nh0yc2feklgd` @ 2026-08-11T17:00:00Z (18:00 BST). Connector verified (`@velacodexyz` present); no velacode IG in Postiz → carousel skipped `ig-pending`, IG payload staged in ledger. Images hosted on branch `claude/optimistic-feynman-1y0x9i` (raw 200 pre-upload). **Gap:** drops #033–#054 were never announced by this routine (memory jumps #032 → #055); this run announces only the newest drop #055 per the routine's mandate. Forge #033–#054 separately if wanted (idempotency keys off each run slug). |
